...
def extract_operators_from_box(plan_text):
    operators = []
    ...
    for i, line in enumerate(lines):
        ...
        while True:
            ...
            if end_idx < 0:
                break
            cols.append((idx, end_idx))
            pos = end_idx + 1
        if cols:
            box_top_cols.append((i, cols))

    for line_idx, cols in box_top_cols:
        next_idx = line_idx + 1
        if next_idx >= len(lines):
            continue
        next_line = lines[next_idx]
        for start, end in cols:
            segment = next_line[start:end + 1] if end + 1 <= len(next_line) else next_line[start:]
            content = segment.replace('│', '').strip()
...
def main():
    ...
    for qname, sql in TPCH_QUERIES.items():
        try:
            result = con.execute(f"EXPLAIN {sql}").fetchall()
            plan_text = result[0][1] if result else ""
            ops = extract_operators_from_box(plan_text)
            plans[qname] = {"plan": plan_text, "operators": ops, "error": None}
        except Exception as e:
            plans[qname] = {"plan": None, "operators": [], "error": str(e)}
