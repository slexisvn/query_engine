...
describe('TPC-H Full Query Suite', () => {
  it('Q1 - Pricing Summary Report', async () => {
    ...
    const total = result.rows.reduce((s, r) => s + r.count_order, 0);
    expect(total).toBeGreaterThan(0);
    expect(total).toBeLessThanOrEqual(tables.LINEITEM.rowCount());
  });
  ...
  it('Q13 - Customer Distribution', async () => {
    ...
    `);
    expect(result.rows.length).toBeGreaterThan(0);
    const totalCust = result.rows.reduce((s, r) => s + r.custdist, 0);
    expect(totalCust).toBe(tables.CUSTOMER.rowCount());
  });

  it('Q14 - Promotion Effect', async () => {
    const result = await engine.run(`
  ...
  it('Q16 - Parts/Supplier Relationship (COUNT DISTINCT)', async () => {
    const result = await engine.run(`
    ...
    for (const row of result.rows) {
      expect(row.supplier_cnt).toBeGreaterThan(0);
    }
  });

  it('Q19 - Discounted Revenue (complex OR predicates)', async () => {
    const result = await engine.run(`
  ...
  it('Q2 - Minimum Cost Supplier', async () => {
    const result = await engine.run(`
  ...
  it('Q8 - National Market Share', async () => {
    const result = await engine.run(`
      SELECT o_year,
  ...
  it('Q9 - Product Type Profit Measure', async () => {
    const result = await engine.run(`
  ...
  it('Q11 - Important Stock Identification', async () => {
    const result = await engine.run(`
  ...
  it('Q15 - Top Supplier (using CTE)', async () => {
    const result = await engine.run(`
      WITH revenue AS (
  ...
  it('Q17 - Small-Quantity-Order Revenue', async () => {
    const result = await engine.run(`
  ...
  it('Q18 - Large Volume Customer', async () => {
    const result = await engine.run(`
  ...
  it('Q20 - Potential Part Promotion', async () => {
    const result = await engine.run(`
  ...
  it('Q21 - Suppliers Who Kept Orders Waiting', async () => {
    const result = await engine.run(`
  ...
  it('Q22 - Global Sales Opportunity', async () => {
    const result = await engine.run(`
  ...
  it('verifies nation-region referential integrity', async () => {
    const result = await engine.run(`
      SELECT COUNT(*) AS cnt FROM nation JOIN region ON n_regionkey = r_regionkey
    `);
    expect(result.rows[0].cnt).toBe(25);
  });

  it('verifies date interval arithmetic consistency', async () => {
    const result = await engine.run(`
      SELECT COUNT(*) AS before_cnt FROM orders WHERE o_orderdate < DATE '1995-01-01'
    `);
    const result2 = await engine.run(`
      SELECT COUNT(*) AS after_cnt FROM orders WHERE o_orderdate >= DATE '1995-01-01'
    `);
    expect(result.rows[0].before_cnt + result2.rows[0].after_cnt).toBe(tables.ORDERS.rowCount());
  });
});
