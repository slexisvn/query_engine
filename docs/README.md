...
The chapters follow the TypeScript code in [src/](../src/): a SQL frontend, optimizer, execution operators, columnar storage, and the parallel and distributed layers. Treat this as a study of a concrete implementation. Some paths are experimental, some registered choices share an implementation, and SQL support has limits. The relevant chapters distinguish the general idea from what this engine currently does.

## Who this is for

You can program. You have written SQL, or at least read some. You have never built a database and do not know what a "plan node" is. The book builds toward reading plans, explaining optimizer choices, and making a small change to a pass or operator. You do not need to complete the implementation exercises to follow the main argument.

## How to read it

For a first pass, read chapters **1, 2, 4, 10, 13, 17, 29, 30, and 34**. This route follows data through a query before asking you to implement a parser or optimizer. Chapter 3 is a source map to revisit as needed.

For a complete reading, continue in chapter order. Parts 1–2 explain translation, Part 3 explores optimization, Parts 4–5 explain execution and storage, and Parts 6–7 cover scaling and the user-facing tools. Use the [glossary](appendix/glossary.md) when a term is unfamiliar.

Each chapter offers a short understanding check, observation exercises, and optional extensions. The hints give you a way to check your reasoning. Experiments that alter the engine should run on a local branch; revert deliberate breakage before continuing.
...
The running query uses [three customers](examples/customer.csv) and [four orders](examples/orders.csv). It returns Alice with a total of 350 and Carol with 300. Chapters introduce additional datasets when an algorithm needs a different shape; larger cardinalities are separate experiments, not the size of these CSVs.
...
| # | Chapter |
|---|---|
| 50 | [The DataFrame API](07-surfaces/50-dataframe-api.md) |
| 51 | [Tools: CLI, REPL, and the visualizer](07-surfaces/51-tools.md) |
| 52 | [Testing a query engine](07-surfaces/52-testing.md) |
| 53 | [Debugging a wrong plan](07-surfaces/53-debugging.md) |
| 54 | [WASM: loading, dispatch, and execution](07-surfaces/54-wasm.md) |

## Appendix

- [SQL dialect reference](appendix/sql-grammar.md)
