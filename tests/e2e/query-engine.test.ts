...
describe('QueryEngine', () => {
  ...
  describe('join on a derived table or CTE whose column name collides (regression)', () => {
    const TS = [{ name: 'id', dataType: 'INT32' }, { name: 'parent', dataType: 'INT32' }];
    const US = [{ name: 'id', dataType: 'INT32' }];

    function engineWithCollidingNames() {
      const catalog = new Catalog();
      registerMockTable(catalog, 'T', TS, [makeChunk([
        { type: 'INT32', values: [1, 2, 3, 4] },
        { type: 'INT32', values: [null, 1, 2, 2] },
      ])]);
      registerMockTable(catalog, 'U', US, [makeChunk([
        { type: 'INT32', values: [1, 2, 3, 4] },
      ])]);
      return new QueryEngine(catalog);
    }

    it('joins a derived table on the key instead of degenerating to a cross product', async () => {
      const engine = engineWithCollidingNames();
      const result = await engine.run(
        'SELECT u.id FROM u JOIN (SELECT id FROM t) s ON u.id = s.id ORDER BY u.id'
      );
      expect(result.rows).toEqual([{ id: 1 }, { id: 2 }, { id: 3 }, { id: 4 }]);
      engine.close();
    });

    it('joins a derived table that filters, matching only the surviving keys', async () => {
      const engine = engineWithCollidingNames();
      const result = await engine.run(
        'SELECT u.id FROM u JOIN (SELECT id FROM t WHERE id > 2) s ON u.id = s.id ORDER BY u.id'
      );
      expect(result.rows).toEqual([{ id: 3 }, { id: 4 }]);
      engine.close();
    });

    it('joins a CTE on a key that shares its name with an outer column', async () => {
      const engine = engineWithCollidingNames();
      const result = await engine.run(
        'WITH s AS (SELECT id FROM t WHERE id > 1) SELECT a.id FROM t a JOIN s ON a.parent = s.id ORDER BY a.id'
      );
      expect(result.rows).toEqual([{ id: 3 }, { id: 4 }]);
      engine.close();
    });

    it('joins the same CTE to itself under two aliases', async () => {
      const engine = engineWithCollidingNames();
      const result = await engine.run(
        'WITH s AS (SELECT id FROM t) SELECT a.id FROM s a JOIN s b ON a.id = b.id ORDER BY a.id'
      );
      expect(result.rows).toEqual([{ id: 1 }, { id: 2 }, { id: 3 }, { id: 4 }]);
      engine.close();
    });

    it('keeps the derived alias when the subquery ends in LIMIT rather than a projection', async () => {
      const engine = engineWithCollidingNames();
      const result = await engine.run(
        'SELECT u.id FROM u JOIN (SELECT id FROM t ORDER BY id LIMIT 2) s ON u.id = s.id ORDER BY u.id'
      );
  ...
  describe('spilling does not change results', () => {
    const ROW_SCHEMA = [DataType.INT32, DataType.INT32, DataType.INT32];
    const savedFlush = Config.flushBatchSize;
    let restoreMemoryLimit;

    beforeEach(() => { restoreMemoryLimit = captureMemoryLimit(); });
    afterEach(() => { restoreMemoryLimit(); Config.flushBatchSize = savedFlush; });
    ...
    function bigTable() {
      ...
      for (let start = 0; start < ids.length; start += 20) {
        chunks.push(makeChunk([
          { type: 'INT32', values: ids.slice(start, start + 20) },
          { type: 'INT32', values: ks.slice(start, start + 20) },
          { type: 'INT32', values: vs.slice(start, start + 20) },
        ]));
      }
      registerMockTable(catalog, 'A', schema, chunks);
      return new QueryEngine(catalog);
    }
    ...
    for (const sql of [
      ...
      it(`matches in-memory vs spilled: ${sql.slice(0, 32)}...`, async () => {
        ...
        limitResidentRows(ROW_SCHEMA, 4); Config.flushBatchSize = 2;
