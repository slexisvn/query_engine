...
import { createDefaultOptimizer } from '../../src/optimizer/optimizer-pipeline.js';
import { PlanNodeType, JoinType } from '../../src/planner/logical-plan.js';
import { PhysicalNodeType, isPhysicalJoin } from '../../src/execution/physical-plan.js';
import { PhysicalPlanner } from '../../src/execution/physical-planner.js';

function physicalJoins(plan, statistics) {
  const physical = new PhysicalPlanner(statistics || new Map()).plan(plan);
  const joins = [];
  const stack = [physical];
  while (stack.length > 0) {
    const node = stack.pop();
    if (isPhysicalJoin(node)) joins.push(node);
...
function physicalJoinTypes(plan, statistics) {
  ...
  while (stack.length > 0) {
    const node = stack.pop();
    if (isPhysicalJoin(node)) types.push(node.type);
    for (const child of node.children) stack.push(child);
  }
  return types;
}
...
function createOptimizer(statistics = null) {
...
function joinStrategies(plan, statistics) {
  return physicalJoinTypes(plan, statistics);
}

function physicalNodeTypes(plan, statistics, predicate) {
  const physical = new PhysicalPlanner(statistics || new Map()).plan(plan);
  const types = [];
  const stack = [physical];
  while (stack.length > 0) {
    const node = stack.pop();
    if (predicate(node)) types.push(node.type);
    for (const child of node.children) stack.push(child);
  }
  return types;
}
...
function physicalAggregateTypes(plan, statistics) {
  return physicalNodeTypes(plan, statistics, node => AGGREGATE_TYPES.has(node.type));
}
...
describe('TPC-H Optimizer E2E', () => {
  ...
  describe('Q1 - Pricing Summary', () => {
    ...
    it('assigns a physical aggregate operator', () => {
      const plan = optimizeSQL(sql);
      expect(physicalAggregateTypes(plan).length).toBeGreaterThan(0);
    });
  });
  ...
  describe('cross-query optimization invariants', () => {
    ...
    it('all joins get a physical operator', () => {
      for (const [, sql] of Object.entries(ALL_QUERIES)) {
        const plan = optimizeSQL(sql);
  ...
  describe('cost-based optimization with TPC-H statistics', () => {
    ...
    describe('join reorder with cost model', () => {
      it('reorders joins in Q5 (6-table join) based on cardinality', () => {
        ...
        expect(physicalJoins(plan, tpchStats()).length).toBe(joins.length);
      });
      ...
      it('prefers nested loop for tiny build side, hash for larger tables', () => {
        ...
        const joins = findNodes(plan, PlanNodeType.JOIN);
        expect(joins.length).toBe(1);
        expect(physicalJoinTypes(plan, tpchStats())[0]).toBe(PhysicalNodeType.NESTED_LOOP_JOIN);
    ...
    describe('physical strategy selection based on cost', () => {
      ...
      it('ungrouped aggregate gets UNGROUPED strategy', () => {
        ...
        `);
        const aggs = findNodes(plan, PlanNodeType.AGGREGATE);
        expect(physicalAggregateTypes(plan, tpchStats())[0]).toBe(PhysicalNodeType.UNGROUPED_AGGREGATE);
      });
      ...
      it('grouped aggregate with low-NDV keys gets PERFECT_HASH', () => {
        ...
        `);
        const aggs = findNodes(plan, PlanNodeType.AGGREGATE);
        expect(physicalAggregateTypes(plan, tpchStats())[0]).toBe(PhysicalNodeType.PERFECT_HASH_AGGREGATE);
      });
      ...
      it('low-NDV group key can use PERFECT_HASH strategy', () => {
        const plan = optimizeWithStats(`
          SELECT O_ORDERSTATUS, COUNT(*) FROM ORDERS GROUP BY O_ORDERSTATUS
        `);
        expect([PhysicalNodeType.PERFECT_HASH_AGGREGATE, PhysicalNodeType.HASH_AGGREGATE])
          .toContain(physicalAggregateTypes(plan, tpchStats())[0]);
      });
    ...
    describe('build side selection for all join types', () => {
      it('LEFT join uses right as build side', () => {
        const plan = optimizeWithStats(`
          ...
          LEFT JOIN ORDERS o ON c.C_CUSTKEY = o.O_CUSTKEY
        `);
        for (const physical of physicalJoins(plan, tpchStats())) {
          if (physical.logical.joinType !== JoinType.LEFT) continue;
      ...
      it('SEMI join uses right as build side', () => {
        ...
        `);
        for (const physical of physicalJoins(plan, tpchStats())) {
          if (physical.logical.joinType !== JoinType.SEMI) continue;
          expect(physical.buildSide).toBe('right');
        }
      });
      ...
      it('INNER join picks smaller side as build', () => {
        ...
        `);
        const joins = physicalJoins(plan, tpchStats()).filter(j => j.logical.joinType === JoinType.INNER);
        expect(joins.length).toBeGreaterThan(0);
        for (const join of joins) {
          expect(join.buildSide).toBeDefined();
        }
      });
    });

    describe('sort-merge join consideration', () => {
      it('merge joins carry an explicit sort requirement', () => {
        const plan = optimizeWithStats(`
          SELECT * FROM ORDERS o
          JOIN LINEITEM l ON o.O_ORDERKEY = l.L_ORDERKEY
        `);
        const physical = new PhysicalPlanner(tpchStats()).plan(plan);
        const stack = [physical];
        while (stack.length > 0) {
          const node = stack.pop();
          if (node.type === PhysicalNodeType.MERGE_JOIN) {
            expect(typeof node.requiresSort.left).toBe('boolean');
            expect(typeof node.requiresSort.right).toBe('boolean');
          }
          for (const child of node.children) stack.push(child);
        }
      });

      it('every join gets a physical operator', () => {
        const plan = optimizeWithStats(`
          SELECT * FROM CUSTOMER c
          JOIN ORDERS o ON c.C_CUSTKEY = o.O_CUSTKEY
          JOIN LINEITEM l ON o.O_ORDERKEY = l.L_ORDERKEY
        `);
        for (const type of physicalJoinTypes(plan, tpchStats())) {
          expect([PhysicalNodeType.HASH_JOIN, PhysicalNodeType.MERGE_JOIN, PhysicalNodeType.NESTED_LOOP_JOIN]).toContain(type);
        }
      });
    });
