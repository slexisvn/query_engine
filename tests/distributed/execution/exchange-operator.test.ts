...
describe('ExchangeSender', () => {
  ...
  it('hash-shuffle sends each row to the correct target based on hash', async () => {
    ...
    const secondPass = new MockTransport();
    const repeated = new ExchangeSender(secondPass, ['node-0', 'node-1'], {
      exchangeType: 'hash_shuffle',
      keyExtractors: [(chunk, row) => chunk.columns[0].get(row)],
      partitionCount: 2,
      channelId: 'ch-3b',
    });
    await repeated.consume(makeIntChunk([...values].reverse()));

    const repeatedByNode = { 'node-0': new Set(), 'node-1': new Set() };
    for (const msg of secondPass._sent) {
      const decoded = codec.decode(msg.data);
      for (let i = 0; i < decoded.size; i++) {
        repeatedByNode[msg.targetNodeId].add(decoded.columns[0].get(i));
      }
    }

    for (const node of ['node-0', 'node-1']) {
      expect([...repeatedByNode[node]].sort()).toEqual([...rowsByNode[node]].sort());
    }
  });
