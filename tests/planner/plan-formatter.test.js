...
import {
  ...
  LogicalIndexScan, JoinType, PhysicalStrategy, PlanNodeType,
...
describe('formatPlan', () => {
  ...
  it('Join: Hash prefix for HASH strategy, Merge for MERGE, none for others', () => {
    const hash = LogicalJoin(JoinType.INNER, null, LogicalScan('A', []), LogicalScan('B', []), PhysicalStrategy.HASH);
    expect(formatPlan(hash)).toContain('Hash Join');

    const merge = LogicalJoin(JoinType.INNER, null, LogicalScan('A', []), LogicalScan('B', []), PhysicalStrategy.MERGE);
    expect(formatPlan(merge)).toContain('Merge Join');
  });
  ...
  it('Join: INNER omits type prefix, LEFT/RIGHT/FULL show type', () => {
    ...
    expect(formatPlan(inner)).toContain('Hash Join');
  ...
  it('Aggregate: Hash/Stream/Ungrouped/PerfectHash prefixes', () => {
    const h = LogicalAggregate([], [], LogicalScan('T', []), PhysicalStrategy.HASH);
    expect(formatPlan(h)).toContain('Hash Aggregate');

    const s = LogicalAggregate([], [], LogicalScan('T', []), PhysicalStrategy.STREAM);
    expect(formatPlan(s)).toContain('Stream Aggregate');

    const u = LogicalAggregate([], [], LogicalScan('T', []), PhysicalStrategy.UNGROUPED);
    expect(formatPlan(u)).toContain('Ungrouped Aggregate');

    const p = LogicalAggregate([], [], LogicalScan('T', []), PhysicalStrategy.PERFECT_HASH);
    expect(formatPlan(p)).toContain('Perfect Hash Aggregate');
  });
  ...
  it('complex plan: full readable output for Hash Join with Filter', () => {
    ...
    expect(lines[1]).toContain('Hash Aggregate');
    expect(lines[1]).toContain('group by: U.NAME');
    expect(lines[2]).toContain('Filter (condition: (U.ACTIVE = true))');
    expect(lines[3]).toContain('Hash Join (condition: (U.ID = O.USER_ID))');
