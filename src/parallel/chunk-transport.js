...
import { isSharedView } from '../storage/sab-arena.js';
...
function copyColumn(col, size, allocator) {
  ...
  if (isFixedWidth(col.dataType)) {
    ...
    data.set(col.data.subarray(0, size));
...
export function shareChunk(chunk, columnIndexes, arena, forceCopy = false) {
...
function readDictBlob(lengths, count, blob) {
  const reverseDict = new Array(count);
  let offset = 0;
...
export function encodeChunkSet(chunks, columnIndexes, arena, dictCache = new Map()) {
  const buffers = [];
...
export function transportBytes(chunkSet) {
  let total = 0;
