...
export function writePartialSpill(file, partitions) {
  const buffers = partitions.map(partition => partition.length > 0 ? v8.serialize(partition) : null);
...
export function readPartialSpill(file, partition) {
