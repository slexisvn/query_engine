...
interface CatalogLike {
  registerTable(name: string, schema: ColumnSchema[], options: RegisterTableOptions): void;
}

function col(name: string, dataType: DataType): ColumnSchema {
...
const NATION_COLUMNS: ColumnSchema[] = [
...
const PART_COLUMNS: ColumnSchema[] = [
...
const PARTSUPP_COLUMNS: ColumnSchema[] = [
...
const CUSTOMER_COLUMNS: ColumnSchema[] = [
...
const ORDERS_COLUMNS: ColumnSchema[] = [
...
const LINEITEM_COLUMNS: ColumnSchema[] = [
...
export const TPCH_TABLES: Record<string, TableSchema> = {
...
export function createTPCHCatalog(): Catalog {
  const catalog: Catalog = new Catalog();
  const registrar: CatalogLike = catalog;
  for (const [name, def] of Object.entries(TPCH_TABLES)) {
    registrar.registerTable(name, def.columns, {
