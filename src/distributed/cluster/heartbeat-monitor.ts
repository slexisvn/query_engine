...
export class HeartbeatMonitor {
  ...
  private _windows: Map<NodeId, ArrivalWindow>;
  private _timer: ReturnType<typeof setInterval> | null;
  private _sendFn: HeartbeatSendFn | null;
  private _statusCallback: NodeStatusChange | null;
...
class ArrivalWindow {
  ...
  private _meanAccum: number;
  private _varianceAccum: number;
  ...
  _addInterval(interval: number): void {
  ...
  _removeFromStats(value: number): void {
    this._meanAccum -= value;
    this._varianceAccum -= value * value;
  }

  _computePhi(elapsed: number, mean: number, stddev: number): number {
