...
import { nullsFirstFor, compareOrderedValues } from '../../execution/operators/sort.js';
import type {
  NodeId,
  ChannelId,
  DistributedOrderKey,
...
export class MergeExchangeOperator {
  ...
  _compareRows(chunkA: ReadableChunk, rowA: number, chunkB: ReadableChunk, rowB: number): number {
    for (const key of this._orderKeys) {
      ...
      const cmp = compareOrderedValues(valA, valB, key.direction, nullsFirstFor(key.direction, key.nullOrder));
      if (cmp !== 0) return cmp;
    }
    return 0;
  }
