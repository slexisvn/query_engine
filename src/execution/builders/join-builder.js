...
export async function buildJoin(executor, node) {
  ...
  return {
    ...
    register: (graph, currentPipelineId, currentSink) => {
      ...
      graph.setSource(currentPipelineId, async function* () {
        ...
        const countRows = (chunks) => chunks.reduce((sum, c) => sum + c.size, 0);
        ...
        const typesOf = (side, chunks) => {
          if (side.storage) return side.spec.schema.map(col => col.dataType);
          const first = chunks.find(c => c.size > 0);
          if (first) return first.columns.map(col => col.dataType);
          return side.spec.schema.map(col => col.dataType);
        };
        const outputTypes = {
...
export function prepareParallelJoin(executor, node, buildInput, probeInput, buildNode, probeNode, buildKeys, probeKeys, residualCondition, combinedMapping) {
...
export async function runBufferedSerialJoin(makeBuildSide, makeProbeOp, buildChunks, probeChunks, node, probeColCount) {
  const buildSide = makeBuildSide();
  ...
  if (probeOp.finalize) {
    await probeOp.finalize({ consume: async (chunk) => { out.push(chunk); } });
  }
  return out;
}
