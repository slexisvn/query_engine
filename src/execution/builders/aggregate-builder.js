...
export async function buildAggregate(executor, node) {
...
export async function buildPartialAggregate(executor, node) {
  const child = await executor.buildPipeline(node.children[0]);
...
export async function buildFinalAggregate(executor, node) {
  const child = await executor.buildPipeline(node.children[0]);
