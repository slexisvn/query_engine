...
export function joinKeyOf(extractors, chunk, rowIdx) {
...
export function joinKeyHash(key) {
  return hashValue(key);
}

export function createCombinedRowAdapter(totalCols) {
...
export function probeJoinRows(items, lookup, opts) {
...
export function emitsOnUnmatchedProbe(joinType) {
...
export function emitsUnmatchedBuild(joinType) {
  return joinType === JoinType.LEFT || joinType === JoinType.FULL;
}

export function buildJoinOutputChunk(rows, { joinType, buildColCount, buildSchema, probeSchema }, allocator = heapAllocator) {
  if (rows.length === 0) {
...
export function materializeRow(chunk, rowIdx) {
