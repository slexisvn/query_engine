...
const OP = { SUM: 0, COUNT: 1, COUNT_STAR: 2, AVG: 3, MIN: 4, MAX: 5 };
...
export function createVectorAggregator(spec) {
...
function makeAggState(op, capacity) {
  switch (op) {
    case OP.SUM:
...
function growAggState(op, state, capacity) {
...
function exportSlotState(op, state, slot) {
  switch (op) {
    case OP.SUM:
...
export class VectorGroupAggregator {
  constructor(keyKind, groupIdx, aggs) {
