...
export function compileVectorExpression(expr, columnMapping) {
  if (!expr) return null;

  switch (expr.kind) {
    case BoundExprKind.COLUMN_REF: {
      const colIdx = resolveColIdx(expr, columnMapping);
      return (chunk) => ({ ref: true, colIdx, column: chunk.columns[colIdx] });
    }

    case BoundExprKind.LITERAL: {
      const val = expr.value;
      return (chunk) => ({ ref: false, constant: true, value: val, size: chunk.size });
    }
...
export function vectorGet(result, chunk, i) {
...
export function vectorizedFilter(chunk, scalarEvalFn) {
...
export function vectorizedHashProbe(probeChunk, probeKeyFn, hashTable, maxMatches = 65536) {
  const buildRefs = [];
  const probeIndices = [];
...
export function buildJoinOutputDirect(buildChunks, buildRefs, probeChunk, probeIndices, buildColCount, probeColCount) {
...
export function vectorizedProject(chunk, evaluators, expressions, resultTypes) {
  const outputCols = [];
...
function createEvalColumn(chunk, evalFn, dataType) {
...
function resolveColIdx(expr, columnMapping) {
  if (columnMapping) {
...
function compileVectorBinaryOp(op, leftFn, rightFn) {
  return null;
}

function vectorUnaryMinus(operand, chunk) {
  return null;
}

function vectorUnaryNot(operand, chunk) {
  return null;
}
