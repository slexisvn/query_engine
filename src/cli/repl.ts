...
export function startREPL(engine: QueryEngine, options: ReplOptions = {}): void {
  ...
  rl._insertString = function(this: ReadlineInternal, c: string): void {
  ...
  rl.on('line', async (line: string) => {
    ...
    if (queryBuffer.endsWith(';')) {
      try {
        ...
        let result: Awaited<ReturnType<QueryEngine['run']>> | Awaited<ReturnType<QueryCoordinator['execute']>>;
...
async function handleMetaCommand(cmd: string, engine: QueryEngine, coordinator: QueryCoordinator | null): Promise<void> {
